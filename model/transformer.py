...
class Transformer(nn.Module):
    ...
    def act_aware_random_masking(self, x, cam, mask_ratio, tau):
        """
        ...
        cam = cam.permute(0, 2, 3, 1).contiguous()
        cam = cam.mean(dim=[3])
        cam = cam.reshape(NM, L)
        cam = cam / (torch.max(cam, dim=-1, keepdim=True).values * tau + 1e-10)
        cam_prob = F.softmax(cam, dim=-1)

        noise = torch.log(cam_prob) - torch.log(-torch.log(torch.rand(NM, L, device=x.device) + 1e-10) + 1e-10)  # gumble
    ...
    def forward_encoder(self, x, mask_ratio, motion_aware_tau, cam):
        ...
        if cam is not None:
            x, mask, ids_restore, _ = self.act_aware_random_masking(x, cam, mask_ratio, motion_aware_tau)
    ...
    def forward_loss(self, imgs, pred, mask, cam, tau):
        """
        imgs: [NM, T, V, 3]
        pred: [NM, TP * VP, t_patch_size * patch_size * 3]
        mask: [NM, TP * VP], 0 is keep, 1 is remove
        cam:  [NM, 1, TP, VP], body part weights from CAM,
        """
        ...
        cam = cam.permute(0, 2, 3, 1).contiguous() # [NM, TP, VP, 1]

        cam = cam.mean(dim=[3]) #[NM, TP, VP]

        cam = cam.reshape(NM, TP_VP)
        ...
        #  # 计算加权损失
        # weighted_loss = (loss * mask * cam_softmax).sum() / (mask * cam_softmax).sum()

        mask_loss = (loss * mask).sum() / mask.sum()  # mean loss on removed joints

        # total_loss = (weighted_loss + mask_loss) / 2.0

        return mask_loss

    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, cam = None, alpha=0.2, **kwargs):
        """
        cam: [64, 1, 30, 25]
        """
        N, C, T, V, M = x.shape
        ...
        x_acceleration = self.extract_motion(x_motion, motion_stride)


        latent, mask, ids_restore = self.forward_encoder(x, mask_ratio, motion_aware_tau, cam)
        pred = self.forward_decoder(latent, ids_restore)  # [NM, TP * VP, C]

        loss1 = self.forward_loss(x_motion, pred, mask, cam, motion_aware_tau)

        loss2 = self.forward_loss(x_acceleration, pred, mask, cam, motion_aware_tau)

        loss =   (1 - alpha ) * loss1 + alpha * loss2
        # loss = self.forward_loss(x, pred, mask, cam, motion_aware_tau)
...
class MaskSem(nn.Module):
    def __init__(self, dim_in=3, dim_feat=256, decoder_dim_feat=256,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, norm_skes_loss=False,
                 mamp = None,
                 encoder = None, encoder_args = None,
                 finetune = None,momentum = 0.999, queue_size=32768):
        super().__init__()
        ...
        if mamp is not None:
            self.Mamp = import_class(mamp)
        else:
            raise ValueError("mamp must be provided")
        
        self.mamp = self.Mamp(dim_in=dim_in,dim_feat=dim_feat,decoder_dim_feat=decoder_dim_feat,
                 depth=depth, decoder_depth=decoder_depth, num_heads=num_heads, mlp_ratio=mlp_ratio,
                 ...
                 drop_path_rate=drop_path_rate, norm_layer=norm_layer, norm_skes_loss=norm_skes_loss)
        if finetune:
            self.checkpoint = torch.load(finetune, map_location='cpu')
            print("Load pre-trained checkpoint from: %s" % finetune)
            self.checkpoint_mamp = self.checkpoint['model']
            # interpolate position embedding

        self.mamp.load_state_dict(self.checkpoint_mamp, strict=False)

        self.msg = self.mamp.load_state_dict(self.checkpoint_mamp, strict=False)
        print(self.msg)


        if encoder is not None:
            self.Encoder = import_class(encoder)
        else:
            raise ValueError("mamp must be provided")
        if hasattr(self, 'Encoder'):
            self.encoder = self.Encoder(**encoder_args)
        else:
            raise RuntimeError("Failed to load encoder class")
        

        self.encoder_params_dict = {name: param for name, param in self.encoder.named_parameters()}
        self.mamp_params_dict = {name: param for name, param in self.mamp.named_parameters()}



        for name, param in self.encoder.named_parameters():
            if name in self.encoder_params_dict:
                param.data.copy_(self.mamp_params_dict[name].data)  # 更新参数值
                param.requires_grad = False  # 设置 requires_grad 为 False

        # create the queue
        self.register_buffer("queue", torch.randn(dim_feat, queue_size))
    ...
    def _momentum_update_key_encoder(self):
        ...
        for name, param in self.encoder.named_parameters():
            if name in self.encoder_params_dict:
                param.data = param.data * self.m + self.mamp_params_dict[name].data * (1.0 - self.m)
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, alpha=0.2, **kwargs):
        with torch.no_grad():
            self._momentum_update_key_encoder()
            x_mean = x.mean(dim=0, keepdim=True) #N, C, T, V, M [32, 3, 120, 25, 2]
            k_mean_feat = self.encoder(x_mean)  #N, C [1, 256]
            _ , cam = self.encoder(x,key=True, mean_feat=k_mean_feat)
        loss, pred, mask = self.mamp(x, mask_ratio, motion_stride, motion_aware_tau, cam, **kwargs)
        return loss, pred, mask
