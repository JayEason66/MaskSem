...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 ...
                 intra_p=0.5, inter_p=0.0,
                 uniform=False, aug_method='z',model = "pretrain"):
        """
        data_path:
        label_path:
    ...
    def __getitem__(self, index):
        ...
        if self.uniform:
            data_numpy, index_t = tools.valid_crop_uniform(data_numpy, valid_frame_num, self.p_interval, self.window_size, 64)
        else:
            data_numpy, index_t = tools.valid_crop_resize(data_numpy, valid_frame_num, self.p_interval, self.window_size, 64)
        if self.random_rot:
        ...
        if self.split == 'train':
            ...
            elif not (self.model == "pretrain") and (p < (self.intra_p + self.inter_p)) & (p >= self.intra_p):
